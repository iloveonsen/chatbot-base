...
def get_chatbot_response(user_message, session, owner):
    ...
    prompt = PromptTemplate.from_template(_DEFAULT_TEMPLATE).partial(persona=persona)
    chain = RunnableParallel({"history": retriever | format_docs , "사용자": RunnablePassthrough()}) | prompt | llm | StrOutputParser()
    ...
    if verbose:
        ...
        print(RunnableParallel({"history": retriever | format_docs , "사용자": RunnablePassthrough()}).invoke(user_message))

    retriever.add_documents([Document(page_content=f'사용자:{user_message}')]) # metadata 추가 가능
